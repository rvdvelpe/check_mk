...
def getbmvdata():
	...
	conn = pypyodbc.connect('Driver=FreeTDS;Server=172.20.5.42;port=1441;uid=OmdPTelecomSql;pwd=Omd;database=bmvd0001')
	cur = conn.cursor()
	cur.execute('''Select
	...
	FROM relation R
		INNER JOIN Object O1 ON (R.Object1ID = O1.ID and o1.IsArchived=0) 
		INNER JOIN ObjectType OT1 ON O1.ObjectTypeID = OT1.ID
		...
		inner join Object O2 ON (R.Object2ID = O2.ID and o2.IsArchived=0)
		INNER JOIN ObjectType OT2 ON O2.ObjectTypeID = OT2.ID
	...
	SELECT
		...
		o.ID as 'Eigenschap Value'
	FROM Object o 
		INNER JOIN ObjectType ot ON (o.ObjectTypeID = ot.ID and o.IsArchived=0)
	...
	for middel in bmv:
		...
		if hostgroup in hostgroups:
			...
			if ipaddress and bmv[middel]['Status'] == "Actief - Operationeel":
				folders[hostgroup].append((middel1,alias,ipaddress,interf))
...
def checkobjects(bmv):
	...
	for hostgroup in bmv:
		for name, s_alias, ipaddress, interf in bmv[hostgroup]:
...
def createfiles(bmv):
	...
	for folder in bmv:		
		...
		for name, s_alias, ipaddress, interf in bmv[folder]:
